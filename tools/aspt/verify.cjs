...
const scope=JSON.parse(read('aspt/scope-audit.json'));
assert.equal(scope.lessons.length,19);
for(const l of D.lessons){assert.equal(l.bodyRefs.length,l.body.length);for(const ref of l.bodyRefs)assert(ref.length>10);}
for(const row of scope.practice.excluded)assert(!D.lessons.some(l=>l.bank.includes(row.bankIndex)));
assert(!D.lessons.some(l=>l.visuals.includes('v012')||l.visuals.includes('v027')));
...
for(const entry of ['index.html','cmt-quiz.html']){
 ...
 const focusedChecks=await page.evaluate(()=>{let attempts=0;for(const l of ASPT.data.lessons){ASPT.practice(l.id);const qs=session.questions.map(q=>q.ref);for(const q of qs){if(q.studyLegacy?.includes('action-potential'))throw Error('advanced diagram leaked');for(let pick=0;pick<q.o.length;pick++){launch([q],'ASPT scope check');const cur=session.questions[0],choice=cur.opts.findIndex(o=>o.text===q.o[pick]);chooseAnswer(choice);if(session.score!==(pick===q.a?1:0))throw Error('focused score '+q._id);if(!document.querySelector('#qexplain').textContent.includes(q.e))throw Error('focused feedback '+q._id);attempts++;}}}return attempts;});assert(focusedChecks>700);
