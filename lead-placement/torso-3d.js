...
window.LeadTorso3D = (() => {
  ...
  async function mount(host,options={}) {
    ...
    let active=options.active||"v1",labelActive=options.activeLabel||active,labelMode=options.labelMode||"all",showAllLabels=options.showAllLabels??labelMode!=="guided",layers={body:true,skeleton:true,muscle:false,landmarks:true,leads:true};
    ...
    function highlight(ids){const selected=new Set(Array.isArray(ids)?ids:[ids]);highlighted=selected;active=[...selected][0]||"";host.dataset.highlight=[...selected].join(",");markerItems.forEach((item)=>{const on=selected.has(item.id),primary=item.id===labelActive;item.group.scale.setScalar(on?1.38:.82);item.group.children.forEach((o)=>{if(o.material)o.material.opacity=on?1:.45;});item.button.dataset.active=String(primary);item.button.dataset.dimmed=String(!primary);item.button.setAttribute("aria-current",primary?"true":"false");});landmarkItems.forEach((item)=>{const on=selected.has(item.userData.id);item.material.opacity=on?1:.2;item.scale.setScalar(on?1.6:1);});structureMeshes.forEach((mesh)=>{const on=[...selected].some((id)=>matchesStructure(mesh,id)||mesh.userData.structure===id);mesh.material.emissive.setHex(on?0xffa600:0x000000);mesh.material.emissiveIntensity=on?.58:0;});draw();}
    function setLabels(next={}){if(next.mode)labelMode=next.mode;if("all" in next)showAllLabels=!!next.all;if("active" in next)labelActive=next.active||"";markerItems.forEach((item)=>{const primary=item.id===labelActive;item.button.dataset.active=String(primary);item.button.dataset.dimmed=String(!primary);item.button.setAttribute("aria-current",primary?"true":"false");});host.dataset.labelMode=labelMode;host.dataset.allLabels=String(showAllLabels);host.dataset.activeLabel=labelActive;draw();}
