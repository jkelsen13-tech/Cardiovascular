...
window.LeadTorso3D = (() => {
  ...
  async function mount(host,options={}) {
    ...
    function candidatePositions(base,id,family,width,height,boxWidth,boxHeight){const preferred={v1:[-30,-25],v2:[30,-27],v3:[34,-9],v4:[30,15],v5:[34,-20],v6:[36,16],ra:[-32,-8],la:[32,-8],rl:[-28,18],ll:[28,18]}[id]||[0,-28],extra=family==="limb"?54:48;const [dx,dy]=preferred,offsets=[[dx,dy],[dx,dy-extra],[dx,dy+extra],[dx,dy-extra*2],[dx,dy+extra*2],[dx-extra,dy],[dx+extra,dy],[dx-extra,dy-extra],[dx+extra,dy+extra],[0,-extra],[0,extra],[0,-extra*2],[0,extra*2]],stepX=Math.max(56,boxWidth+12),stepY=Math.max(56,boxHeight+12);for(let ring=1;ring<=6;ring+=1){for(let gx=-ring;gx<=ring;gx+=1){offsets.push([gx*stepX,-ring*stepY],[gx*stepX,ring*stepY]);}for(let gy=-ring+1;gy<ring;gy+=1){offsets.push([-ring*stepX,gy*stepY],[ring*stepX,gy*stepY]);}}const halfW=boxWidth/2+4,halfH=boxHeight/2+4;return offsets.map(([x,y])=>({x:clamp(base.x+x,halfW,width-halfW),y:clamp(base.y+y,halfH,height-halfH),width:boxWidth,height:boxHeight}));}
