...
window.LeadTorso3D = (() => {
  ...
  async function mount(host,options={}) {
    ...
    function projectLessonLabels(){const rect=viewport.getBoundingClientRect(),placed=[];lessonLabels.forEach((item)=>{const show=layers.landmarks&&item.modes.includes(lessonMode);item.label.hidden=!show;if(!show)return;const p=worldPoint(item.point);p.project(camera);if(p.z>1||p.z< -1||Math.abs(p.x)>1.08||Math.abs(p.y)>1.08){item.label.hidden=true;return;}item.label.hidden=false;item.label.style.visibility="hidden";item.label.style.transform="none";const width=Math.max(54,item.label.offsetWidth),height=Math.max(24,item.label.offsetHeight),anchorX=(p.x*.5+.5)*rect.width,anchorY=(-p.y*.5+.5)*rect.height,preferLeft=item.label.dataset.side==="left",shifts=[0,-height-4,height+4,-2*(height+4),2*(height+4)];let chosen=null;for(const shift of shifts){const left=clamp(preferLeft?anchorX-width-10:anchorX+10,4,rect.width-width-4),top=clamp(anchorY+shift-height/2,24,rect.height-height-4),box={left,right:left+width,top,bottom:top+height};if(!placed.some((other)=>box.left<other.right&&box.right>other.left&&box.top<other.bottom&&box.bottom>other.top)){chosen=box;break;}}if(!chosen){item.label.hidden=true;return;}placed.push(chosen);item.label.style.left=chosen.left+"px";item.label.style.top=chosen.top+"px";item.label.style.visibility="";item.label.dataset.collision="placed";});host.dataset.lessonLabels=String(placed.length);}
